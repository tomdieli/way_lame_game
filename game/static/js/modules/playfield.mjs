...
export class GameTable extends PIXI.Application {
  constructor(localPlayerName) {
    ...
    this.stage.addChild(this.gameBoard.getMap());
  ...
  movePlayer = (e) => {
    ...
    case 0:
      ...
      const targetHex = this.gameBoard.Grid.pointToHex(offsetX, offsetY);
    ...
    case 2:
      ...
      break;
    default:
      console.log(`Sorry, we are out of ${data.button}.`);
      return;
    }
    const neighbors = this.gameBoard.getNeighborHexes(player.getHex(), player.getDirection());
    ...
    this.stage.children.forEach(chi => {
      if((chi instanceof PlayerPiece) && (chi.name !== player.name)) {
        ...
        for(const neighbor of neighbors) {
          ...
          const engagables = this.gameBoard.getNeighborHexes(chi.getHex(), chi.getDirection());
